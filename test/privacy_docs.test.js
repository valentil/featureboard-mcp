...
test("findOpenWorldTools finds the known egress-capable tools (canary for parser drift)", () => {
  const names = findOpenWorldTools(serverSrc);
  for (const known of ["notify_slack", "deploy_site", "commit_feature", "get_site_traffic", "check_updates", "activate_license"]) {
...
test("PRIVACY.md's Exceptions section exists and covers more than a single item", () => {
  ...
  for (const name of ["notify_slack", "deploy_site", "commit_feature", "get_site_traffic", "register_email", "check_updates", "activate_license"]) {

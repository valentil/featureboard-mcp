  ...
  "set_project_config",
  {
    ...
    inputSchema: {
      ...
      checks: z.object({
        ...
        testImpact: z.object({
          command: z.string().optional().describe("Delegated mode: the repo's own impact runner (e.g. 'npm run test:impact'), run once with FB_IMPACT_CHANGED (newline-separated changed files) and FB_IMPACT_REVISION in the env. When omitted, builtin mode runs each impacted test entry directly with node."),
          entries: z.array(z.string()).optional().describe("Explicit test entry files (repo-relative). Default: every `node <path>.js` invocation discovered in package.json scripts (excluding bare `node --check`)."),
          slowPatterns: z.array(z.string()).optional().describe("Impacted entries matching these substrings/prefixes are reported as deferred (run in CI) instead of run locally — corpus audits, benchmarks, browser suites."),
          ignorePatterns: z.array(z.string()).optional().describe("Changed files matching these are excluded from impact computation (generated bundles, docs, ...)."),
          graphPath: z.string().optional().describe("Where the impact-graph database lives. Default: <project>/.featureboard/checks/impact-graph.json — built/refreshed incrementally by every run; format {version:1, entries, files:{rel:{m,s,deps}}}."),
          timeoutMinutes: z.number().optional().describe("Delegated: overall command timeout (default 10). Builtin: per-test timeout (default 5)."),
        }).optional().describe("FBMCPF-387: impact testing — run ONLY the tests whose require/import dependency closure contains a changed file; the full suite belongs in CI. Impacted tests also surface in get_work_packet.impactedTests."),

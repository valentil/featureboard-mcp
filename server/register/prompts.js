    ...
    messages: [
      {
        ...
        content: {
          ...
          text:
            ...
            "4. Do the work. If it's a substantial or code ticket, dispatch it to a fresh sub-agent with the packet so it gets isolated context — at the model from the ticket's model: label (haiku/sonnet/opus may run in parallel; fable inline with review), with rigor matched to its effort: label. Do trivial tickets inline. Tell the sub-agent to call log_heartbeat a few times at milestones during longer dispatches so get_agent_monitor shows live progress instead of a blank wait. Only you (the orchestrator) write to the board.\n" +
    ...
    messages: [
      {
        ...
        content: {
          ...
          text:
            ...
            "3. Dispatch: for every ticket in dispatch.parallel (haiku/sonnet/opus), set_status In Progress, get_work_packet, and start a sub-agent at that model with the packet as its brief — these can run in parallel. When parallel tickets touch DISJOINT code areas, give each its own isolated git worktree (create_worktree) so agents don't edit the shared repo at once, then merge branches back SERIALLY and cleanup_worktree. Work dispatch.sequential tickets (fable) one at a time, inline in the orchestrator, with a review between tickets.\n" +
    ...
    messages: [
      {
        ...
        content: {
          ...
          text:
            ...
            "4. Offer to start wave 1 now via the daily-plan dispatch flow: run daily_plan to stamp model:/effort: labels, then dispatch a sub-agent per ticket at its model/effort tier (haiku/sonnet/opus in parallel, fable inline). Only advance to the next wave once its blockers are Done. Wait for my go-ahead before dispatching.\n",
        },
      },
    ],
  })

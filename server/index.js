...
import { routingScorecard } from "./routingscore.js";
...
import { getTicketHistory, agentMonitorV2, appendEvent, getTimelineData, appendHeartbeat, completedAtForTask, lastDispatchForTicket } from "./events.js";
import { evaluateRules } from "./rules.js";
import { postProjectUpdate, getLatestUpdate, UPDATE_HEALTH, checkUpdates } from "./updates.js";
import { lintVoice, getVoiceProfile, maybeLint } from "./voice.js";
import { getPricing, rollupCost } from "./pricing.js";
import { applyStandard, resolveStandard, standardPacketBlock } from "./standards.js";
import { steerProject, getSteeringStatus } from "./steering.js";
import { addKbDoc, listKbDocs, getKbDoc, searchKb, recordLearning } from "./kb.js";
import { addSource, addSourceFromUrl, addSourceFromFile, listSources, getSource } from "./sources.js";
import { ragSearch, ragSearchHybrid, warmEmbeddings } from "./rag.js";
import { prepareResearch, appendResearch } from "./research.js";
import {
...
import {
  ...
  listSiteTemplates, applySiteTemplate, unmanagedSite,
...
import { blendStatus, blendPlan } from "./blend.js";
import { resolveChecksConfig, startChecks, getCheckResults, evaluateChecksGate, uncollectedCheckRuns } from "./checks.js";
// FBMCPF-361: wave dispatch — the plural of next_task (see server/wave.js).
import { buildWave } from "./wave.js";
import { getLiveActivity } from "./liveactivity.js";
...
// FBMCPF: RAG explorer artifact (get_rag_explorer) — shipped alongside the board UI.
const RAG_EXPLORER_HTML_PATH = nodePath.join(SERVER_DIR, "..", "artifact", "rag-explorer.html");
...
3. Step 0: check the packet's gitTargets — code commits and projectpad commits can go to DIFFERENT repos; never assume. SATURATE THE LANES: call next_wave (not repeated next_task) whenever more than one ticket is open. It returns the whole dispatchable set at once, already partitioned into file-disjoint lanes — start EVERY lane it returns as a concurrent sub-agent. No lane sits idle while a dispatchable ticket exists. The instant a lane finishes, call next_wave again with occupied=[still-running tickets] and refill that lane in the SAME turn — never batch completions and refill later, and never pause mid-run to ask whether to keep going. Tickets WITHIN a lane share files and run serially; sequential[] (fable) runs inline in the orchestrator. next_task remains for the single-ticket case (it honours manual priority). set_status <ticket> "In Progress" BEFORE you start. Call get_work_packet to assemble a focused brief (scope, linked issue, code location, custom prompt, definition of done) and read the files it points to rather than dumping them. Each packet and next_task response now carries a \`dispatch\` block ({subAgent, model, cap, parallelizable, instruction}) — obey it: fan haiku/sonnet/opus tickets out to parallel sub-agents when parallelizable and file-disjoint; keep fable in the orchestrator. DISPATCH BY TICKET ID, NOT BY VALUE (FBMCPB-83): the prompt you pass to a sub-agent is rendered verbatim in the user's chat, so NEVER paste a work packet, researchBrief, ragChunks or next_wave JSON into it — name the ticket and tell the sub-agent to call get_work_packet itself as its first action. Sub-agents may READ the board via get_work_packet; they still write nothing but log_heartbeat. Keep the transcript quiet: one line per dispatch (ticket, model, lane), one line per wave, and ONE consolidated report at the end — not a per-ticket essay. For a substantial or code ticket, dispatch it to a fresh sub-agent so it works in isolated context; do trivial tickets inline. Pick the sub-agent\u2019s model from the ticket\u2019s model: label (or the packet\u2019s suggestedModel): haiku/sonnet/opus tickets may run as PARALLEL sub-agents; fable tickets run INLINE in the orchestrator with review between. Match rigor to the effort: label — low: minimal exploration, obvious change, verify, stop; medium: normal loop with tests; high: read adjacent code, protect invariants and back-compat, add tests, self-review the diff. Dispatches running more than a couple minutes should show live progress, not just a generic \"multitasking\" wait: tell the sub-agent to call log_heartbeat a few times at natural milestones (a short phase note, plus model/elapsedMinutes/spend when known) — get_agent_monitor and the board's live/stall banners read these per ticket. As the orchestrator, emit your own one-line status when checking in on a running dispatch (ticket, model, elapsed, cap spend) so the user sees progress even before you next touch the board. Sub-agents NEVER write the board — the orchestrator alone sets status, logs work, and commits. Only you (the orchestrator) write to the board. When finished, set_status "Done" with a one-line completionSummary AND log_work with additions/deletions (and model) so progress is recorded. Write tools return compact acks by default (verbose:true for the full ticket) — do not re-fetch tickets you just wrote. If git is configured for the project, commit the change per ticket (commit_feature, message referencing the ticket id) before pulling the next. Then pull the next. (The process_next prompt runs this loop for you.)
...
PROJECT STANDARD: every work packet carries a standard block — the project's rigor profile (prototype | standard | polished) with directives and any project mandate. Hold the work to that bar. If a packet's standard shows locked:false and the user's cues make the expected rigor clear (e.g. 'highly polished', 'just a quick prototype'), call set_standard ONCE with source "inferred" to lock it in, then stop revisiting. A locked standard is settled: never re-infer or change it unless the user explicitly asks — then set_standard with force:true.

STEERING (never stop at an empty queue): an empty wave is a refill event, not an ending — next_wave's stopCondition, not an empty queue, is the authority on whether you are done. When next_wave/next_task returns nothing and stopCondition.met is false, call steer_project. It returns ordered passes: review the last wave's Done tickets for real defects (file bugs), triage hygiene findings, research toward the project's goal at its standard and file the next wave of features, then resume next_task. Execute the passes; only stop when the user says stop, or when two consecutive steering passes report nothing actionable.

DOGFOODING (recursive, FBMCPF-323): when you change FeatureBoard's OWN workflow — a tool, a steering pass, a work-packet field, a cleanup heuristic — exercise the change in situ on this board in the same session before calling it done: file it as a ticket and work it through the board, and run the changed surface against real board state (re-run steer_project, call churn_reconcile here, pull a work packet, scan the board). The edit must run through the thing it edits. Green unit tests prove the code does what you told it; dogfooding proves you told it the right thing. See docs/DOCTRINE.md.
...
const CORE_TOOLS = new Set([
  ...
  "get_health", "get_work_log", "get_scratchpad", "set_scratchpad", "next_task", "next_wave",
  ...
  "set_usage_type", "register_email", "check_updates",
  ...
  // routing_scorecard joins them in core because the analytics overlay's routing
  // panel (FBMCPF-369) calls it — it degrades to a "not available" card without it.
  "estimate_work", "plan_budget", "daily_plan", "routing_scorecard",
  ...
  "add_decision", "list_decisions", "set_handoff", "get_ticket_history", "export_audit",
  "get_timeline_data",
  ...
  "add_kb_doc", "append_research", "list_kb_docs", "search_kb", "get_kb_doc", "rag_search", "get_rag_explorer",
  "add_source", "list_sources", "get_source",
  // project standard / rigor profile + steering loop
  "set_standard", "steer_project", "get_steering_status",

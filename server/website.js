...
/**
 * FBMCPB-34: marker stamped into every generated page. persist() refuses to
 * overwrite an existing index.html that lacks it (a hand-built shipped site),
 * so deploy_site can never clobber a production homepage with a generated stub.
 */
export const GENERATED_MARKER = "<!-- generated by FeatureBoard site tools -->";
...
/**
 * FBMCPB-34 clobber guard: if the site dir already has an index.html that was
 * NOT generated by these tools (no GENERATED_MARKER), refuse to touch anything.
 * Returns a non-throwing refusal object, or null when writing is allowed
 * (no existing index.html, marker present, or force). Never throws.
 */
function clobberGuard(dir, force) {
  if (force) return null;
  const htmlPath = path.join(dir, SITE_HTML);
  let existing;
  try {
    existing = fs.readFileSync(htmlPath, "utf8");
  } catch {
    return null; // no existing index.html — first write into this dir is fine
  }
  if (existing.includes(GENERATED_MARKER)) return null;
  return {
    skipped: true,
    reason:
      `Refusing to overwrite ${htmlPath}: the existing index.html was not generated by ` +
      `FeatureBoard site tools (generator marker missing — likely a hand-built site). ` +
      `Nothing was written (config, index, or sub-pages). Pass force:true to overwrite.`,
    htmlPath,
 ...
 * FBMCPB-34: refuses (returns { skipped:true, ... }) instead of overwriting a
 * hand-built index.html that lacks GENERATED_MARKER, unless opts.force is set.
 */
function persist(board, project, cfg, now, { force = false } = {}) {
  const dir = siteDir(board, project);
  const refusal = clobberGuard(dir, force);
...
export function addPage(board, project, { slug, title, sections, seo } = {}, { now = new Date() } = {}) {
  ...
  if (summary.skipped) return summary; // FBMCPB-34: hand-built site — nothing written
...
export function addRawPage(board, project, { slug, title, html } = {}, { now = new Date() } = {}) {
  ...
  cfg.pages = Array.isArray(cfg.pages) ? cfg.pages : [];
  const dir = siteDir(board, project);
  // FBMCPB-34: don't drop raw sub-pages into a hand-built site dir either.
  const refusal = clobberGuard(dir, false);
  if (refusal) return { project, ...refusal };
  fs.mkdirSync(dir, { recursive: true });
...
export function removePage(board, project, slug, { now = new Date() } = {}) {
  ...
  cfg.pages = next;
  const summary = persist(board, project, cfg, now);
  if (summary.skipped) return summary; // FBMCPB-34: hand-built site — don't unlink either
...
export function setPageSeo(board, project, { slug, ...fields } = {}, { now = new Date() } = {}) {
  ...
  if (slug == null || slug === "" || slug === "index") {
    cfg.seo = { ...(cfg.seo || {}), ...seo };
    const summary = persist(board, project, cfg, now);
    if (summary.skipped) return summary; // FBMCPB-34
